...
class Task1 : Task
{
    ...
    private void Do_The_Task(string text, out int _res)
    {
        ...
        var letterFrequency = new int[33];
        _res = 0;
        foreach (var word in words)
        {
            foreach (char letter in word.ToLower())
            {
                ...
                {
                    int index = letter - 'а'; // индекс буквы в массиве
        ...
        char mostFrequentLetter = (char)('а' + maxIndex);

        // Выводим слова, содержащие самую часто встречающуюся букву
        Console.WriteLine($"Самая часто встречающаяся буква: {mostFrequentLetter}");

        // Собираем слова, содержащие самую часто встречающуюся букву, в массив
        ...
        string [] wordsWithMostFrequentLetter = wordsList.ToArray();
        foreach (var word in wordsWithMostFrequentLetter)
        {
            if (word.ToLower().Contains(mostFrequentLetter))
            {
    ...
    public override string ToString()
    {
        return _res.ToString() + "  = Количество слов с самой часто встречающейся буквой";
    }
